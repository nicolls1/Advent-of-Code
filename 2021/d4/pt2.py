...
def run():
  ...
  for number in input_numbers:
    ...
    for idx, board in enumerate(board_found_number_positions):
      if idx not in won_boards and board_has_winning_combination(board):
        won_boards.append(idx)

        if len(won_boards) == len(boards):
          losing_board = boards[won_boards[-1]]
          for position in board:
            losing_board[position] = 0
          remaining_numbers_sum = sum(losing_board)

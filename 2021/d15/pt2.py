...
def sum_digits(number):
  value = sum([int(value) for value in str(number)])
  if value < 10:
    return value
  else:
...
def run():
  ...
  expand_factor = 5
  expanded_data = [[] for _ in range(original_height * expand_factor)]
  for expand_y in range(5):
    for expand_x in range(5):
      for index, row in enumerate(data):
        expanded_data[index + (original_height * expand_y)] += [sum_digits(value + expand_x + expand_y)
                                                                for value in row]
  data = expanded_data

...
def find_max_geode(blueprint: blueprint):
  ...
  while len(queue) > 0:
    ...
    if (current_state.as_hash() in seen_states and seen_states[current_state.as_hash()] <= current_state.time) \
        or (MINUTES - current_state.time + 1) * (MINUTES - current_state.time + 2) / 2 + \
    ...
    for robot_name, robot_build_resources in blueprint.items():
      if set(robot_build_resources.keys()).issubset(income_ores.keys()):
        ...
        if current_state.time + mine_time < MINUTES:
          ...
          queue.append(State(
            ...
            {
              ROBOT_TO_ORE[robot]: current_state.resources[ROBOT_TO_ORE[robot]] + count * (mine_time + 1) -
...
def run():
  ...
  results = [find_max_geode(blueprint) for blueprint in robot_blueprints]
  print(sum([(idx + 1) * result for idx, result in enumerate(results)]))
